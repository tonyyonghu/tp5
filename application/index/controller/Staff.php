...
use think\Upload;
ob_end_clean();
		class Staff extends LoginTrue{

			public function Add(){
				$this->LoginTrue();
				$depId=$_GET['depId'];
				$rs_staffNum=Db::name('staff')->count();
		        ...
		        if($depId!=0){
		        	$rs_department=Db::name("department")->column("dId,dPid,dPsid,dName")->where("dId={$depId}")->find();
		        }else{
		        	$rs_department=Db::name("department")->column("dId,dPid,dPsid,dName")->select();
		        }
		        $this->assign("depId",$depId);
		        $this->assign("rs_department",$rs_department);
		        //下面对自定义变量的处理
		        // $variables=M("variables");
		        $rs_xueli=Db::name('variables')->where("vId=1")->find();
		        $xueli=explode("|",$rs_xueli["vVariablesVal"]);
		        $this->assign("xueli",$xueli);
		        
		        $rs_zhicheng=Db::name("variables")->where("vId=2")->find();
		        $zhicheng=explode("|",$rs_zhicheng["vVariablesVal"]);
		        $this->assign("zhicheng",$zhicheng);
		        
		        $rs_zhiwu=Db::name("variables")->where("vId=3")->find();
		        $zhiwu=explode("|",$rs_zhiwu["vVariablesVal"]);
		        $this->assign("zhiwu",$zhiwu);
		        
		        $rs_hunyin=Db::name("variables")->where("vId=4")->find();
		        $hunyin=explode("|",$rs_hunyin["vVariablesVal"]);
		        $this->assign("hunyin",$hunyin);
		        
		        $rs_minzu = Db::name("variables")->where("vId=5")->find();
