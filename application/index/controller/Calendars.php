...
use think\Db;
	...
	class Calendars extends LoginTrue{
  ...
  public function NowToday(){
     ...
     // $staff=M("staff");
      ...
      $rs_glBNum=Db::name("staff")->where($glmap)->count();
      $this->assign("rs_glBNum",$rs_glBNum);
      if($rs_glBNum>0){
      $rs_glB=Db::name("staff")->where($glmap)->select();
      ...
      $rs_nlBNum=Db::name("staff")->where($nlmap)->count();
      $this->assign("rs_nlBNum",$rs_nlBNum);
      if($rs_nlBNum>0){
          $rs_nlB=Db::name("staff")->where($nlmap)->select();
